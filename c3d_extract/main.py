...
def main():
    ...
    with torch.no_grad():
        ...
        for name, unit in video_loader:
            ...
            visual_feature = visual_feature.reshape(visual_feature.shape[1]*visual_feature.shape[0],)
            # print("VSS", visual_feature.shape)
            visual_feature = nn.Linear(4096 * int(args.frame_unit)//16 ,4096).to('cuda')(visual_feature)
            visual_feature = visual_feature.reshape(-1, visual_feature.shape[0])

            visual_activity_concept = visual_activity_concept.reshape(visual_activity_concept.shape[1]*visual_activity_concept.shape[0],)
            visual_activity_concept = nn.Linear(487 * int(args.frame_unit)//16, 487).to('cuda')(visual_activity_concept)
            visual_activity_concept = visual_activity_concept.reshape(-1, visual_activity_concept.shape[0])
            ...
            # print("FEATURE SHAPE, ", visual_feature.shape, visual_activity_concept.shape)
